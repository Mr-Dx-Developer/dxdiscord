...
const log = {
    ...
    write: (content, { color = "\x1b[37m", tag = "LOG", error = false } = {}) => {
        ...
        stream.write(`\x1b[1;36m[dxdiscord]\x1b[0m[${log.timestamp()}]${color}[${tag}]: ${log.clean(content)}\x1b[0m\n`);
        return false;
    },
    ...
    /** Special case log handler for dxdiscord specifc logs
     *  Or just outright void spammy logging (heartbeats)
     * @param {string|object} err - error to process */
    handler: (type, err) => {
        const e = err.toString();
        if (e.includes("[DISALLOWED_INTENTS]")) log.error("YOU DIDN'T ENABLE INTENTS - go back to the dxdiscord readme.md and read the section under \"setup\"");
    ...
    /** Special case error handler for dxdiscord specifc errors
...
const sendStaffChatMessage = (z, name, msg) => {
    ...
    getPlayers().forEach(async function(player, index, array) {
        if (IsPlayerAceAllowed(player, "dxdiscord.staffchat")) {
