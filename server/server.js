...
SetConvarReplicated("dxdiscord_servername", z.config.FiveMServerName);
SetConvarReplicated("dxdiscord_discordinvite", z.config.DiscordInviteLink);
SetConvarReplicated("dxdiscord_serverip", z.config.FiveMServerIP);
SetConvarReplicated("dxdiscord_userpresence", String(z.config.enableUserPresence));
...
on("playerJoining", (oldId) => {
    ...
    if (z.bot.isRolePresent(member, z.config.StaffChatRoleIds)) {
        ExecuteCommand(`add_principal "player.${source}" group.dxdiscordstaff`);
    }
});
...
if (z.config.EnableStaffChatForwarding) {
    RegisterCommand("staff", (source, args, raw) => {
        if (!IsPlayerAceAllowed(source, "dxdiscord.staffchat")) return;
    ...
    RegisterCommand("stafftoggle", (source, args, raw) => {
        if (IsPlayerAceAllowed(source, "dxdiscord.staffchat")) {
            ExecuteCommand(`remove_principal "player.${source}" group.dxdiscordstaff`);
